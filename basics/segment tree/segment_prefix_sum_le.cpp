/*
Find first i elements such that their greatest sum <= x.
Eg. 2 4 3 7 9 6
    2 6 9 16 25 31
	x = 18
	First i elements = 2 4 3 7 i.e. 16 <= 18
	Answer is index that is: 3
*/
...
int kth(vector<int > &t, int tl, int tr, int k, int v = 1) {
//	if(k > t[v])
//		return -1;
	...
	if(t[2*v] >= k || (mid + 1 == tr && (k - t[2*v]) < t[2*v + 1])) {
		return kth(t, tl, mid, k, 2*v);
	} else {
		return kth(t, mid+1, tr, k - t[2*v], 2*v + 1);
	}
}
