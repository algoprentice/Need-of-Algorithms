...
int sum(vector<int > &t, int ql, int qr, int tl, int tr, int v = 1) {
	if(ql > qr) 
		return 0;
	if(ql == tl && qr == tr)
		return t[v];
	...
	int mid = tl + (tr - tl) / 2;
	return sum(t, ql, min(qr, mid), tl, mid, 2*v) + sum(t, max(mid+1, ql), qr, mid+1, tr, 2*v + 1);
}
